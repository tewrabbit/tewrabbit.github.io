$(function () {
    ...
    if (lazyVideos.length > 0 && 'IntersectionObserver' in window) {
        ...
        function enqueue(video) {
            ...
            if (video.dataset.lazyQueued === '1' || video.dataset.lazyLoaded === '1') return;
            video.dataset.lazyQueued = '1';
            queue.push(video);
            pump();
        }
        ...
        function startLoad(video) {
            ...
            setTimeout(() => {
                video.removeEventListener('loadeddata', onReady);
                if (video.dataset.lazyLoaded !== '1') done(video);
            }, 6000);
        ...
        function pump() {
            while (loadingCount < maxConcurrent && queue.length > 0) {
                const video = queue.shift();
                if (!(video instanceof HTMLVideoElement)) continue;
                if (video.dataset.lazyLoaded === '1') continue;
                loadingCount++;
                startLoad(video);
            }
        }
